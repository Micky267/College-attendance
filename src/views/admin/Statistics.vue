<template>
  <div class="statistics">
    ...
    <el-table stripe :data="courseList" border style="width: 100%" :default-sort = "{prop: 'cGrade', order: 'descending'}" max-height="520px" >
      ...
      <el-table-column prop="lateRate" label="迟到率" width="90" sortable>
        <template  slot-scope="scope">
            <span>{{scope.row.lateRate|toRateFn}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="truantRate" label="旷课率" width="90" sortable>
        <template  slot-scope="scope">
          <span>{{scope.row.truantRate|toRateFn}}</span>
      </template>
      </el-table-column>
      <el-table-column prop="leaveRate" label="请假率" width="90" sortable>
        <template  slot-scope="scope">
          <span>{{scope.row.leaveRate|toRateFn}}</span>
      </template>
      </el-table-column>
  ...
  methods: {
    ...
    reqCourse(){
      ...
      let params = {
        ...
        semester:this.selectData.semester.trim(),
        cName: this.selectData.cName.trim(),
        tName: this.selectData.tName.trim(),
        grade:this.selectData.grade.trim(),
      }


      this.$http.get('/api/admin/getAllCoures',{params})
        .then((res)=>{
          this.courseList = res.data.data
