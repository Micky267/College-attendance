  ...
  <div class="nav-side">
    <ul class="wrap-menu" id="menu">
      <li v-for="(items,index) in menuList" :key="index">
        <a href="javascript:" class="link" :class="{'active':curMenuIndex==index&&curSubMenuIndex==-1}"
          @click="onWrapMenu(items,index)">
          <i :class="[items.icon]" class="fa left"></i>
          <span>{{items.title}}</span>
          <i class="fa fa-chevron-right right"
            :class="{'fa-chevron-down':(ifOpen&&curMenuIndex==index&&!items.path)}"></i>
        </a>
        <ul :style="{maxHeight:((ifOpen&&curMenuIndex==index)||(false)?'150px':0)}" class="sub-menu">
          <li v-for="(item,index2) in items.content" :key="index2">
            <a class="link" href="javascript:" :class="{'active':curSubMenuIndex==index2&&curMenuIndex==index}"
              @click="onSubMenu(item,index2)" to="/echat-attend">
